...
char * getExtremaFile(char fileChoice) {
    ...
    struct stat dirStat;
    char *prefFile = NULL;
    int fileSize = (fileChoice == 'L') ? 0: INT32_MAX;
...
char * createDir() {
    ...
    sprintf(nameOfDir, "gonzedua.movies.%i" ,randNum);

    int check = mkdir(nameOfDir, 0750);
    if(check == -1)
        printf("\nError in creating directory.");
    
    printf("\nCreated directory with name %s", nameOfDir);
...
char * promptUserFile() {
    char * userFile = calloc(256, sizeof(char));
...
void promptToProcess() {
    ...
    do{
        ...
        switch(option) {
            case 1:
                fileOption = 'L';
                fileToProcess = getExtremaFile(fileOption);
                break;
            case 2:
                fileOption = 'S';
                fileToProcess = getExtremaFile(fileOption);
